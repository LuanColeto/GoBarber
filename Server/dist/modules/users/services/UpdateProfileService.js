...
let UpdateProfileService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    if (userWithUpdatedEmail && userWithUpdatedEmail.id !== user_id) {
      throw new _AppError.default('This e-mail is already in use');
    }
    ...
    if (password && !old_password) {
      throw new _AppError.default('You need to inform the old password to change your password');
    }

    if (password && old_password) {
      const checkOldPasword = await this.hashProvider.compareHash(old_password, user.password);
